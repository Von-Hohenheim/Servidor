...
function onExtendedOpcode(cid, opcode, buffer)
	...
	if opcode == 103 then
		if t[1] == "Market" then
			...
			if getPlayerItemCount(cid, 2145) >= cost then
				...
				return doPlayerRemoveItem(cid, 2145, cost) and doSendPlayerExtendedOpcode(cid, OPCODE_EMERALD_SHOP, "True")
			else
		...
		elseif t[1] == "ChangeNick" then
			...
			if getPlayerItemCount(cid, 2145) >= 10 then
				if(db.getResult("SELECT `id` FROM `players` WHERE `name` = " .. db.escapeString(newName) .. ";"):getID() == 1) then
					return doPlayerSendCancel(cid, "O nome (" .. newName .. ") já está em uso.")
				end	
				...
				for a = 1, #table do
					if string.find(newName, table[a]) then
						return doPlayerSendCancel(cid, "Este nome (" .. newName .. ") é inválido.")
					end
				end

				if string.len(newName) < 4 then
					return doPlayerSendCancel(cid, "Este nome (" .. newName .. ") é curto demais.")
				elseif string.len(newName) > 15 then
					return doPlayerSendCancel(cid, "Este nome (" .. newName .. ") é longo demais.")
				end
				...
				return doPlayerRemoveItem(cid, 2145, 10)
			else
				return doSendPlayerExtendedOpcode(cid, OPCODE_EMERALD_SHOP, 'You don\'t have enough rubies.')
			end
		...
		elseif t[1] == "Outfits" then
			...
			if getPlayerItemCount(cid, 2145) >= cost then
				...
				if shop.multigender == false and getPlayerSex(cid) ~= shop.gender then
					return doPlayerSendCancel(cid, "Você não pode comprar essa outfit!")
				end
