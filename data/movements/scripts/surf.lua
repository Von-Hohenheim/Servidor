...
function onStepIn(cid, item, position, fromPosition)
	...
	if isBiking(cid) then
		doPlayerSendCancel(cid, "You can't do it while you use the bike!")
...
--[[	if getPlayerStorageValue(cid, 123124) >= 1 and getPlayerStorageValue(cid, 78498) ~= 1 then
		doPlayerSendCancel(cid, "Você não tem licensa para usar o surf!")
		doTeleportThing(cid, fromPosition, false)
		return true
	end			
]]--
	...
	local speed = getPokemonSpeedToSkill(pokeName) * 2
...
function onStepOut(cid, item, position, fromPosition)
	...
	if isPlayer(cid) and isWatchingTv(cid) then return false end
