function onStepIn(cid, item, position, fromPosition)
	if(isPlayer(cid)) and not isRiderOrFlyOrSurf(cid) then
		...
		if getPlayerItemCount(cid, 19219) >= 1 then
			...
			if getPlayerItemCount(cid, 19212) >= 1 and getPlayerItemCount(cid, 19213) >= 1 and getPlayerItemCount(cid, 19214) >= 1 then
				doChangeSpeed(cid, -getCreatureSpeed(cid))
				doChangeSpeed(cid, 800) -- 1500
				doSetCreatureOutfit(cid, {lookType = newtype}, -1)
			end
		end
	end 
	...
	if isWild(cid) or isSummon(cid) then
		...
		elseif poketype1 == "dragon" or poketype2 == "dragon" then
			doChangeSpeed(cid,(-getCreatureSpeed(cid))+360)
		else
			doChangeSpeed(cid,(-getCreatureSpeed(cid))+220)
		end
	end
	return true
end
...
function onStepOut(cid, item, position, fromPosition)
	if not isRiderOrFlyOrSurf(cid) and not isBiking(cid) then
		...
		doChangeSpeed(cid,(-getCreatureSpeed(cid))+400) -- 200, 250
