...
local getNpcTaskName = getPlayerStorageValue(cid, storages.miniQuests.storNpcTaskName2)
local pokeTask1 = getPlayerStorageValue(cid, storages.miniQuests.storPokeNameTask2)
local pokeCountTask1 = tonumber(getPlayerStorageValue(cid, storages.miniQuests.storPokeCountTask2))
local minhaHora = getPlayerStorageValue(cid, storages.miniQuests.storDayTask2)
	
if tonumber(minhaHora) ~= tonumber(hora) then 
	setPlayerStorageValue(cid, storages.miniQuests.storNpcTaskName2, -1)
	setPlayerStorageValue(cid, storages.miniQuests.storPokeNameTask2, -1)
	setPlayerStorageValue(cid, storages.miniQuests.storPokeCountTask2, -1)
...
if ((pokeTask1 == strinSay1 or pokeTask1 == strinSay2) and getNpcTaskName == getCreatureName(getThis())) and msgcontains(string.lower(msg), 'hi') then	
   ...
   if pokeCountTask1 <= 0 then
	   ...
	   setPlayerStorageValue(cid, storages.miniQuests.storPokeNameTask2, "amanha")
...
count = 0
	...
	if((msgcontains(string.lower(msg), 'hi') or  msgcontains(string.lower(msg), 'help')) and (getDistanceToCreature(cid) <= 3)) then
		...
		selfSay('Olá! Minhas tasks são do nivel Medio. Você pode matar ' .. stringToSay2 .. " ou " .. stringToSay1 .. ".")
	...
	elseif msgcontains(string.lower(msg), string.lower(strinSay1)) or msgcontains(string.lower(msg), string.lower(strinSay2)) and getDistanceToCreature(cid) <= 3 and talkState[talkUser] == 1 then	
		...
		if msgcontains(string.lower(msg), string.lower(strinSay1)) then
			setPlayerStorageValue(cid, storages.miniQuests.storNpcTaskName2, getCreatureName(getThis()))
			setPlayerStorageValue(cid, storages.miniQuests.storPokeNameTask2, opcoesDiarias[1][1][1])
			setPlayerStorageValue(cid, storages.miniQuests.storPokeCountTask2, opcoesDiarias[1][1][2])
			setPlayerStorageValue(cid, storages.miniQuests.storDayTask2, hora)
			pokeCounts = opcoesDiarias[1][1][2]
		else
			setPlayerStorageValue(cid, storages.miniQuests.storNpcTaskName2, getCreatureName(getThis()))
			setPlayerStorageValue(cid, storages.miniQuests.storPokeNameTask2, opcoesDiarias[2][1][1])
			setPlayerStorageValue(cid, storages.miniQuests.storPokeCountTask2, opcoesDiarias[2][1][2])
			setPlayerStorageValue(cid, storages.miniQuests.storDayTask2, hora)
...
local tablePoke = diarias.Medio 
local dificult = 2
function onThink()
	-- resetar tasks
	if hora ~= os.date("%d") then
	   opcoesDiarias = {}
	   tablePoke = diarias.Medio 
	   hora = os.date("%d")
	end
