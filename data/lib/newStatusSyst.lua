...
function doCondition2(ret)
	...
	function doBurn2(cid, cd, check, damage)
		...
		--	addEvent(doCreatureAddHealth, 500, cid, -dano, ret.eff, ret.color and ret.color or COLOR_BURN) -- addEvent fazia o dano vir dps do dano da spell daí ficava melhor de ver mas se morrese pra poison bugava o corpo e não dava pra tacar ball
	...
	function doPoison2(cid, cd, check, damage)
		...
		--		addEvent(doCreatureAddHealth, 400, cid, -dano, 8, COLOR_GRASS) -- addEvent fazia o dano vir dps do dano da spell daí ficava melhor de ver mas se morrese pra poison bugava o corpo e não dava pra tacar ball
	...
	function doBuff2(cid, cd, eff, check, buff, first, attr)
		...
		if buff == "Protect" then
			...
			doSendMagicEffect(getThingPos(cid), eff) -- posição default para o ret.effect
		end
		...
		if first then
			...
			elseif isInArray({"Psychic Sight", "Heal Bell", "Future Sight", "Predict", "Camouflage", "Acid Armor", "Iron Defense", "Protect", "Light Screen", "Minimize"}, buff) then
				...
				if not isInArray({"Heal Bell", "Protect", "Light Screen"}, buff) then
					setPlayerStorageValue(cid, 625877, outImune[buff]) -- Protect e Light Screen não mudam a outfit (storage de outfit) 
					doSetCreatureOutfit(cid, {lookType = outImune[buff][getCreatureOutfit(cid).lookType]}, -1) -- mudar outfit caso possua o nome no array (lá em cima)
				end
...
-------------------------------------------
function doMoveDano2(cid, pid, element, min, max, ret, spell) -- add para não receber dano em área se o atacado for boss or shiny.
...
---------------------------------------------------------------------------------
function doDano(cid, element, pos, area, min, max, eff) --Da dano com proteçoes
	if not isCreature(cid) then return true end
	doAreaCombatHealth(cid, element, pos, area, -(math.abs(min)), -(math.abs(max)), eff)
end
---------------------------------------------------------------------------------
