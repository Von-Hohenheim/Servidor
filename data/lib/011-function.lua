...
function doSendOfflineMessage(senderName, targetName, message, itemName, id)
	db.executeQuery(string.format('INSERT INTO `offline_message` (player_name, message, sender_name, item_name, offer_id) VALUES (%s, %s, %s, %s, %s)', db.escapeString(targetName), db.escapeString(message), db.escapeString(senderName), db.escapeString(itemName), db.escapeString(tostring(id))))
	return true
end

function checkOfflineMessages(playerName, offerId)
	local messages = {}
	local str = ""
	local resultId = db.getResult(string.format('SELECT * FROM `offline_message` WHERE `player_name` = %s AND `offer_id` = %s', db.escapeString(playerName), db.escapeString(offerId)))
    if resultId:getID() ~= -1 then
		repeat 
		    local message = resultId:getDataString("message")
			local id = resultId:getDataString("id")
			local sender = resultId:getDataString("sender_name")
			local item = resultId:getDataString("item_name")
			str = str..sender.."*"..message.."|"
...
function checkHistory(playerId)
	local messages = {}
	local str = ""
	local resultId = db.getResult("SELECT `id`, `itemtype`, `amount`, `price`, `expires_at`, `state` FROM `market_history` WHERE `player_id` = "..playerId..";")
    if resultId:getID() ~= -1 then
		repeat 
			local itemName = getItemNameById(getItemClientIdBySpriteId(resultId:getDataInt("itemtype")))
			local price = resultId:getDataInt("price")	
			local amount = resultId:getDataInt("amount")
			local totalPrice = amount*price
			local id = resultId:getDataString("id")
			str = str..itemName.."*"..totalPrice.."*"..price.."*"..amount.."*".."false|"
		until not(resultId:next())
		resultId:free()
		return str
    end
...
function getOfferIdByCounter(T, C)
    local timestamp = T - 30 * 24 * 60 * 60
	local resultId = db.getResult("SELECT `id` FROM `market_offers` WHERE `created` = "..timestamp.." AND (`id` & 65535) = "..C.." LIMIT 1;")
	if resultId:getID() ~= -1 then
		local id = resultId:getDataInt("id")
		resultId:free()
		return id
	end
	return false
...
function deleteMessage(sender, message)
	db.executeQuery(string.format('DELETE FROM `offline_message` WHERE `sender_name` = %s AND `message` = %s', db.escapeString(sender), db.escapeString(message)))
end

function doSendOfflineMessage2(targetName, message)
	db.executeQuery(string.format('INSERT INTO `offline_message2` (player_name, message) VALUES (%s, %s)', db.escapeString(targetName), db.escapeString(message)))
end

function checkOfflineMessage2(cid)
	local resultId = db.getResult(string.format('SELECT * FROM `offline_message2` WHERE `player_name` = %s', db.escapeString(getCreatureName(cid))))
	if resultId:getID() ~= -1 then
		repeat 
			...
			db.executeQuery(string.format('DELETE FROM `offline_message2` WHERE `id` = %d', id))
		until not(resultId:next())
		resultId:free()
	end
end