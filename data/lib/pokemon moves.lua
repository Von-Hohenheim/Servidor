...
local name = getCreatureName(cid)
	 ...
	 if reflectName and reflectName ~= "" then
	  ...
	  elseif name:find("Smeargle") then
		 name = getSmeargleAttackNameFromSkactch(cid, spell) -- retorna o nome do poke q tem essa magia
      end
...
----------------------------min= 1000 + 
	if table ~= "" and table ~= nil then   --alterado v1.6
		...
		if masterLevel >= 10 and masterLevel < 20 then -- 21 ao 29
			mutiplay = 1.1
		elseif masterLevel >= 20 and masterLevel < 30 then -- 30 ao 60
			mutiplay = 1.15
		elseif  masterLevel >= 30 and masterLevel < 40 then -- 61 ao 85
			mutiplay = 1.2
		elseif masterLevel >= 50 and masterLevel < 60 then -- 86 ao 120
			mutiplay = 1.3
		elseif masterLevel >= 70 and masterLevel < 80 then -- 121 ao 180
			mutiplay = 1.4
		elseif  masterLevel >= 90 and masterLevel < 100 then -- 181 ao 220
			mutiplay = 1.5
		elseif masterLevel >= 100 and masterLevel < 125 then -- 221 ao 350
			mutiplay = 1.6
		elseif masterLevel >= 125 and masterLevel < 150 then -- 221 ao 350
			mutiplay = 1.7
		elseif masterLevel >= 150 and masterLevel < 175 then -- 221 ao 350
			mutiplay = 1.8
		elseif masterLevel >= 175 and masterLevel < 200 then -- 221 ao 350
			mutiplay = 1.9
		elseif masterLevel >= 200 and masterLevel < 225 then -- 221 ao 350
			mutiplay = 2
		elseif masterLevel >= 225 and masterLevel < 250 then -- 221 ao 350
			mutiplay = 2.1
		elseif masterLevel >= 250 and masterLevel < 275 then -- 221 ao 350
			mutiplay = 2.2
		elseif masterLevel >= 275 then
			mutiplay = 2.3
		end
		...
		min = getSpecialAttack(cid) * (table.f or math.random(1, 35)) * mutiplay  --alterado v1.6
	    max = min + (isSummon(cid) and getMasterLevel(cid) or getPokemonLevel(cid))
	
	    if spell == "Selfdestruct" then
			if isSummon(target) and getPlayerStorageValue(target, 8979130) >= 1 then
				min = 0
				max = 0
			else
				min = math.ceil(tonumber(getCreatureHealth(cid))*0.8)  
				min = math.ceil(tonumber(getCreatureHealth(cid))*0.8)
			end
        end
		...
		if isNpcSummon(cid) then
			...
			doCreatureSay(getCreatureMaster(cid), getCreatureName(cid)..","..use..""..doCorrectString(spell).."!", 1)
		end
	else
...
--- FOCUS ----------------------------------  
	...
	if getPlayerStorageValue(cid, 253) >= 0 and table ~= "" and table.f ~= 0 then 	
		min = min * 2
		max = max * 2		
...
------ GUST -----
	...
	if getPlayerStorageValue(cid, 5000003) > 0 and table ~= "" and table.f ~= 0 then  
		min = min * 1.25
		max = max * 1.25
		setPlayerStorageValue(cid, 5000003, 0)
	end
...
local nevermiss = {"Spores Reaction", "Zen Mind", "Mirror Coat", "Lifesteal", "Foresight", "Evasion", "Absolute Zero", "Uproar", "Cursed Body", "Shed Skin", "Groundshock", "Sturdy", "Contrary Confusion", "Shadow Counter", "", "Mega Drain", "Lava-Counter", "Counter Helix", "Giroball", "Counter Claw", "Counter Spin", "Demon Kicker", "Demon Punch", "Eletric Charge", "Melody", "Dragon Fury", "Fury", "Static", "Stunning Confusion", "Amnesia", "Healing Wish", "Hex", "Curse", "Guard Split", "Heal Block", "Shock-Counter", "Lava-Counter", "Counter Helix", "Demon Puncher", "Demon Kicker", "Stunning Confusion", "Electric Charge", "Melody", "Mirror Coat", "Mega Drain", "Aromateraphy", "Heal Bell", "Emergency Call", "Magical Leaf", "Sunny Day", "Taunt", "Skull Bash", "Safeguard", "Rain Dance", "Spores Reaction", "Giroball", "Counter Claw", "Counter Spin", "Dragon Fury", "Amnesia", "Zen Mind", "Bone Spin"}
local neverPoke = {"Big Porygon", "Aporygon", "Abporygon"}	
	
	if cd >= 0 or cd2 >= 0 or cd3 >= 0 then                                                         --alterado v1.7
	   if not isInArray(nevermiss, spell) and not isInArray(neverPoke, getCreatureName(cid)) and getPlayerStorageValue(cid, 21100) <= -1 then
			if not string.find(spell, "Mega") then	
				if math.random(1, 100) > 33 then                                                                               --alterado v1.6
...
--------------------HEAL BLOCK------------------------
if getPlayerStorageValue(cid, 5000002) >= 1 and isInArray({"Healarea", "Heal Wish", "Absorb", "Giga Drain", "Rest", "Restore", "Selfheal", "Leech Seed"}, spell) then
		doSendAnimatedText(getThingPosWithDebug(cid), "HEAL BLOCK", 168)
	return false
end				
-----------
     --Posições--
...
--[[	
	...
	elseif spell == "Rollout" then
		...
		local RollOuts = {
			["Voltorb"] = {lookType = 694},
			["Electrode"] = {lookType = 695},
			["Sandshrew"] = {lookType = 637},
			["Sandslash"] = {lookType = 638}, -- por 638
			["Phanpy"] = {lookType = 1005},
			["Donphan"] = {lookType = 1007},
			["Miltank"] = {lookType = 1006}, 
			["Golem"] = {lookType = 639},
			["Blastoise"] = {lookType = 2169},
			["Omastar"] = {lookType = 1579},
			["Shiny Voltorb"] = {lookType = 1387},
			["Shiny Electrode"] = {lookType = 1388},
	...
	elseif spell == "Sketch 2" then	
		local spells = {"Mud Bomb", "Cyber Pulse", "Dark Ball", "Rock Slide", "Magical Leaf", "Fireball", "Waterball", "Feather Dance"}
	...
	elseif spell == "Sketch 3" then
		...
		local spells = {"Poison Gas", "Earthquake", "Petal Tornado", "Electro Field", "Flame Circlel"}
	...
	elseif spell == "Sketch 10" then
		...
		local spells = {"Focus Blast", "Blizzard", "Shadow Storm", "Falling Rocks", "Electric Storm", "Hydro Dance", "Magma Storm", "Draco Meteor", "Psy Impact"}
...
elseif spell == "Dark Eye" or spell == "Miracle Eye" or spell == "Dark Accurate" then -- fazer/editar dark accurate eff 725
	...
	elseif spell == "Psyshock" then 
		...
		function sendAtk2(cid, area, eff)
			if isCreature(cid) then 
				...
				doAreaCombatHealth(cid, psyDmg, area, sand1, -min, -max, 255) --255 --alterado v1.4
			end
		end
		...
		for a = 0, 4 do
			...
			local t = { -- effect
				[0] = {398, {x=p.x+1, y=p.y-(a), z=p.z}}, 
				[1] = {398, {x=p.x+(a+2), y=p.y+1, z=p.z}},
				[2] = {398, {x=p.x+1, y=p.y+(a+2), z=p.z}},
				[3] = {398, {x=p.x-(a), y=p.y+1, z=p.z}}
			}
			local t2 = { -- dano
	...
	elseif spell == "Seed Bomb" then --alterado v1.6
		...
		local function doFall(cid)
			for rocks = 1, 42 do --62
				addEvent(fall, rocks*35, cid, master, GRASSDAMAGE, 1, 503) --54
			end
		end
		...
		addEvent(doFall, 450, cid)
		addEvent(doMoveInArea2, 1400, cid, 2, BigArea2, GRASSDAMAGE, min, max, spell)

		local pid = getSpectators(getThingPos(cid), 7, 7)
		if pid and #pid > 0 then
			for i = 1, #pid do
				if pid[i] ~= cid and ehMonstro(pid[i]) then
					min = min * #pid
					addEvent(doCreatureAddHealth, 1400, cid, min)
					addEvent(doSendAnimatedText, 1400, getThingPosWithDebug(cid), "+"..min.."", 31)
				end
			end
		end
	...
	elseif spell == "Elemental" then
		...
		Types = {
			...
			["psychic"] = {type = psyDmg, eff = 134}, --133
		...
		elemento = pokes[getCreatureName(cid)].type
		dmgType = elemento
		
		if not Types[dmgType] then return true end
		tipo = Types[dmgType]
...
--		addEvent(setPlayerStorageValue, 1400, cid, 32698, -1) -- storage do silence
	...
	elseif spell == "Gyro Ball" then
		...
		local t = {
			{236, {x = p.x+1, y = p.y-1, z = p.z}},
			{232, {x = p.x+2, y = p.y+1, z = p.z}},
			{233, {x = p.x+1, y = p.y+2, z = p.z}},
			{224, {x = p.x-1, y = p.y+1, z = p.z}},
		}
...
elseif spell == "Air Twister" then 
	...
	elseif spell == "Counter Helix" or spell == "Counter Claw" or spell == "Counter Spin" or spell == "Groundshock" then
		...
		local OutFit = {
			...
			["Shiny Hitmontop"] = {out = 1451, cima = 236, direita = 232, esquerda = 224, baixo = 233}, --Shiny Hitmontop 
		...
		local function doDamage(cid, min, max)
			if isCreature(cid) then
				...
				if isInArray({"Scyther", "Shiny Scyther"}, nome1) and sortTypeScyther <= 3 then
					doAreaCombatHealth(cid, null, getThingPos(cid), scythe1, 0, 0, outfitt.cima2) --cima
					doAreaCombatHealth(cid, null, getThingPos(cid), scythe2, 0, 0, outfitt.baixo2) --baixo
					doAreaCombatHealth(cid, null, getThingPos(cid), scythe3, 0, 0, outfitt.direita2) --direita
					doAreaCombatHealth(cid, null, getThingPos(cid), scythe4, 0, 0, outfitt.esquerda2) --esquerda
				else
		...
		local function sendEff(cid)
			if isCreature(cid) then
				if isInArray({"Scyther", "Shiny Scyther"}, nome1) and sortTypeScyther <= 3 then
					doAreaCombatHealth(cid, null, getThingPos(cid), scythe1, 0, 0, outfitt.cima2) --cima
					doAreaCombatHealth(cid, null, getThingPos(cid), scythe2, 0, 0, outfitt.baixo2) --baixo
					doAreaCombatHealth(cid, null, getThingPos(cid), scythe3, 0, 0, outfitt.direita2) --direita --alterado v1.6
					doAreaCombatHealth(cid, null, getThingPos(cid), scythe4, 0, 0, outfitt.esquerda2) --esquerda
				else
					doAreaCombatHealth(cid, null, getThingPos(cid), scythe1, 0, 0, outfitt.cima) --cima
					doAreaCombatHealth(cid, null, getThingPos(cid), scythe2, 0, 0, outfitt.baixo) --baixo
