...
for i = CONST_SLOT_FIRST, CONST_SLOT_LAST do
   ...
   if myball.uid > 0 and isPokeball(myball.itemid) then
...
if isContainer(container) and getContainerSize(container) > 0 then
   for slot = 0, (getContainerSize(container) - 1) do
	...
	if isPokeball(getPlayerSlotItem(cid, 10).itemid) then
		table.insert(ret, getPlayerSlotItem(cid, 10))
		return ret
	end
...
doSendPlayerExtendedOpcode(cid, 95, table.concat(ret))
end
...
local ballname = getItemAttribute(ball.uid, "poke")
...
elseif ballname:find("Smeargle") then
   moves = movestable[getPlayerStorageValue(summon, storages.SmeargleID)]   
else                                                       
		...
		if b then
			if (string.find(b.name, "a - ") and b.mega and b.mega == 1) then
			   if heldy then
				...
				if ballname:find("Smeargle") and getItemAttribute(ball.uid, "sketch" .. a) then
					table.insert(ret, getItemAttribute(ball.uid, "sketchName" .. a)..",")
				else
		     		table.insert(ret, b.name..",")
				end
		   end
...
function doUpdateCooldowns(cid)
	...
	local ballname = getItemAttribute(a.uid, "poke")
	local ret = {}
